...
namespace hooks {
	...
	__int64 __fastcall SetRefreshRate_hook(void* a1, int a2) {
		return config.SetRefreshRate_original(a1, a2);
	}

	void init() {
		MH_Initialize();
		auto IsServer = utils::find_pattern("Taskmgr.exe", "40 53 48 81 EC ? ? ? ? 48 8B 05 ? ? ? ? 48 33 C4 48 89 84 24 ? ? ? ? 83 B9");
		printf("IsServer: 0x%llx\n", IsServer);
